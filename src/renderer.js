...
function updateUpdateView(update = launcherState?.update || {}) {
  ...
  const progress = Math.max(0, Math.min(100, update.progress || 0)); $('currentVersion').textContent = update.currentVersion || '0.4.7'; $('availableVersion').textContent = update.availableVersion || (update.status === 'up-to-date' ? 'Aktuell' : '—'); $('updateBadge').textContent = labels[update.status] || 'Bereit'; $('updateStatus').textContent = messages[update.status] || messages.idle; $('updateProgressBar').style.width = `${progress}%`; $('updateProgressValue').textContent = `${progress} %`;
...
async function removeCustomMod(fileName, button) {
  if (!window.confirm(`Möchtest du „${fileName}“ wirklich aus der Instanz entfernen?`)) return;
  button.disabled = true;
  const result = await window.vortex.removeMod(selectedVersion, fileName);
  if (!result.ok) { addLog(result.error || 'Mod konnte nicht entfernt werden.', 'error'); button.disabled = false; return; }
  addLog(`${fileName} wurde entfernt.`, 'success'); await refreshLibrary();
}
async function toggleCustomMod(fileName, button) {
  button.disabled = true;
  const result = await window.vortex.toggleMod(selectedVersion, fileName);
  if (!result.ok) { addLog(result.error || 'Mod konnte nicht umgeschaltet werden.', 'error'); button.disabled = false; return; }
  addLog(`${fileName.replace(/\.disabled$/, '')} wurde ${result.enabled ? 'aktiviert' : 'deaktiviert'}.`, 'success'); await refreshLibrary();
}
async function refreshLibrary() {
  ...
  $('modList').innerHTML = mods.length ? mods.map(mod => `<div class="mod-row ${mod.enabled === false ? 'mod-disabled' : ''}"><span class="mod-dot ${mod.required ? 'core' : 'custom'}"></span><div><strong>${escapeHtml(mod.name)}</strong><small>${escapeHtml(mod.role || (mod.required ? 'Vortex-Pflichtmod · wird beim Prüfen geschützt' : 'Eigener Mod · in dieser Instanz gespeichert'))}</small></div><span class="mod-badge ${mod.protected ? 'required' : ''}">${mod.protected ? 'COSMETICS-CORE' : mod.required ? 'CORE' : mod.enabled === false ? 'AUS' : 'EIGEN'}</span>${mod.protected ? '<span class="mod-lock" title="Geschützte Vortex-Pflichtmod">🔒</span>' : `<button class="toggle-mod-button ${mod.enabled === false ? '' : 'is-on'}" data-toggle-mod="${escapeAttr(mod.file || mod.name)}" title="${mod.enabled === false ? 'Mod aktivieren' : 'Mod deaktivieren'}" aria-label="${mod.enabled === false ? 'Mod aktivieren' : 'Mod deaktivieren'}"><span></span></button><button class="remove-mod-button" data-remove-mod="${escapeAttr(mod.file || mod.name)}" title="${escapeAttr(mod.name)} entfernen" aria-label="${escapeAttr(mod.name)} entfernen">×</button>`}</div>`).join('') : `<div class="empty-library">Noch keine JAR-Dateien vorhanden. Klicke auf „Instanz reparieren“ oder öffne den Mods-Ordner.</div>`;
  $('modList').querySelectorAll('[data-remove-mod]').forEach(button => button.onclick = () => removeCustomMod(button.dataset.removeMod, button));
  $('modList').querySelectorAll('[data-toggle-mod]').forEach(button => button.onclick = () => toggleCustomMod(button.dataset.toggleMod, button));
...
function renderOnlineMods() {
  ...
  container.innerHTML = onlineModResults.map((mod, index) => { const meta = `${escapeHtml(mod.versionNumber)} · ${escapeHtml(mod.gameVersion)} · ${Number(mod.downloads || 0).toLocaleString('de-DE')} Downloads${mod.author ? ` · ${escapeHtml(mod.author)}` : ''}`; const action = mod.installed ? '<span class="installer-installed">Installiert</span>' : `<button class="button secondary mod-download-button" data-mod-index="${index}">Installieren</button>`; return `<article class="online-mod-card installer-card"><div class="online-mod-icon">${projectIcon(mod)}</div><div class="online-mod-main"><div class="online-mod-title"><h3>${escapeHtml(mod.title)}</h3><span class="mod-badge required">${escapeHtml(mod.versionType || 'release')}</span></div><p>${escapeHtml(mod.description)}</p><small>${meta}</small><span class="installer-dependency-note">Prüft erforderliche Abhängigkeiten automatisch</span></div>${action}</article>`; }).join('');
  bindProjectIcons(container); updatePagination('mods', modSearchState); container.querySelectorAll('[data-mod-index]').forEach(button => button.onclick = () => installOnlineMod(Number(button.dataset.modIndex), button));
}
...
async function installOnlineMod(index, button) {
  const mod = onlineModResults[index]; const version = $('onlineModVersion').value || selectedVersion; if (!mod) return;
  button.disabled = true; button.textContent = 'Installiert …';
  const result = await window.vortex.installModProject(mod.projectId, version);
  if (result.ok) { mod.installed = true; button.replaceWith(Object.assign(document.createElement('span'), { className: 'installer-installed', textContent: 'Installiert' })); const files = [...(result.installed || []), ...(result.present || [])]; addLog(`${mod.title}: ${files.length} Datei(en) inklusive Abhängigkeiten bereitgestellt.`, 'success'); if (result.missing?.length) addLog(`${mod.title}: einige Abhängigkeiten wurden übersprungen.`, 'error'); await refresh(); } else { button.disabled = false; button.textContent = 'Erneut versuchen'; addLog(result.error || 'Mod konnte nicht installiert werden.', 'error'); }
}
...
addLog('Vortex Client Launcher 0.4.7 bereit.'); refresh();
