...
function updateUpdateView(update = launcherState?.update || {}) {
  ...
  $('currentVersion').textContent = update.currentVersion || '0.4.1'; $('updateBadge').textContent = labels[update.status] || 'Bereit'; $('updateStatus').textContent = messages[update.status] || messages.idle; $('updateProgressBar').style.width = `${Math.max(0, Math.min(100, update.progress || 0))}%`;
...
addLog('Vortex Client Launcher 0.4.1 bereit.'); refresh();
