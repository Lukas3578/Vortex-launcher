...
function updateUpdateView(update = launcherState?.update || {}) {
  ...
  const progress = Math.max(0, Math.min(100, update.progress || 0)); $('currentVersion').textContent = update.currentVersion || '0.4.6'; $('availableVersion').textContent = update.availableVersion || (update.status === 'up-to-date' ? 'Aktuell' : '—'); $('updateBadge').textContent = labels[update.status] || 'Bereit'; $('updateStatus').textContent = messages[update.status] || messages.idle; $('updateProgressBar').style.width = `${progress}%`; $('updateProgressValue').textContent = `${progress} %`;
...
addLog('Vortex Client Launcher 0.4.6 bereit.'); refresh();
