...
const MODRINTH_USER_AGENT = 'Lukas3578/Vortex-launcher/0.9.54 (github.com/Lukas3578/Vortex-launcher)';
...
async function searchFreeMusic(query, page = 1) { const normalizedQuery = String(query || '').trim().slice(0, 80); if (!normalizedQuery) throw new Error('Enter an artist, track or mood first.'); const cleanQuery = normalizedQuery.replace(/["\\]/g, ''); const safeQuery = `mediatype:audio AND (title:"${cleanQuery}" OR creator:"${cleanQuery}")`; const endpoint = `https://archive.org/advancedsearch.php?q=${encodeURIComponent(safeQuery)}&fl[]=identifier&fl[]=title&fl[]=creator&fl[]=description&fl[]=licenseurl&rows=18&page=${Math.max(1, Number(page) || 1)}&output=json`; const response = await fetch(endpoint, { headers: { Accept: 'application/json', 'User-Agent': MODRINTH_USER_AGENT }, signal: AbortSignal.timeout(20000) }); if (!response.ok) throw new Error(`Free music search failed (${response.status}).`); const payload = await response.json(); const docs = Array.isArray(payload?.response?.docs) ? payload.response.docs : []; const results = []; for (const item of docs.slice(0, 18)) { const license = String(item.licenseurl || '').toLowerCase(); try { const metaResponse = await fetch(`https://archive.org/metadata/${encodeURIComponent(item.identifier)}`, { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(15000) }); if (!metaResponse.ok) continue; const metadata = await metaResponse.json(); const resolvedLicense = String(item.licenseurl || metadata.metadata?.licenseurl || '').toLowerCase(); if (!resolvedLicense || (!resolvedLicense.includes('creativecommons.org') && !resolvedLicense.includes('publicdomain'))) continue; const file = (metadata.files || []).find(entry => { const name = String(entry.name || '').toLowerCase(); const format = String(entry.format || '').toLowerCase(); return (/\\.(mp3|ogg|oga|wav)$/.test(name) && !name.includes('_files.xml')) || format.includes('vbr mp3') || format === 'ogg vorbis'; }); if (!file?.name) continue; results.push({ id: item.identifier, title: item.title || item.identifier, creator: item.creator || 'Unknown artist', description: String(item.description || '').replace(/<[^>]+>/g, '').slice(0, 180), license: item.licenseurl || metadata.metadata?.licenseurl || 'License shown on source page', sourceUrl: `https://archive.org/details/${encodeURIComponent(item.identifier)}`, audioUrl: `https://archive.org/download/${encodeURIComponent(item.identifier)}/${encodeURIComponent(file.name)}` }); } catch (_) {} } return { ok: true, source: 'Internet Archive · free audio', results }; }
...
async function transferVersionContent(sourceVersion, targetVersion) {
  ...
  const sourceMods = modsRoot(source); const targetPacks = resourcePacksRoot(target); const sourcePacks = resourcePacksRoot(source);
  ensureDir(modsRoot(target)); ensureDir(targetPacks);
  const skipped = []; const mods = []; const resourcePacks = []; const unresolvedMods = [];
  if (exists(sourceMods)) for (const entry of fs.readdirSync(sourceMods, { withFileTypes: true })) {
    ...
    if (mandatoryModNames(source).has(baseName)) { skipped.push(`${entry.name}: Vortex core kept version-specific`); continue; }
    const mapped = await mapInstalledModrinthFile(source, entry.name);
    if (!mapped?.projectId) { unresolvedMods.push(entry.name); skipped.push(`${entry.name}: no Modrinth project mapping`); continue; }
    try {
      const result = await installModrinthProject(mapped.projectId, target);
      if (result?.installed?.length || result?.present?.length) mods.push({ source: entry.name, projectId: mapped.projectId, installed: result.installed || [], present: result.present || [] });
      else { unresolvedMods.push(entry.name); skipped.push(`${entry.name}: no compatible ${target} version`); }
    } catch (_) { unresolvedMods.push(entry.name); skipped.push(`${entry.name}: no compatible ${target} version`); }
  }
  const copyPack = (from, to, label) => { if (exists(to)) { skipped.push(`${label}: already exists`); return false; } const stat = fs.statSync(from); if (stat.isFile() && stat.size > 500 * 1024 * 1024) { skipped.push(`${label}: too large`); return false; } fs.cpSync(from, to, { recursive: true, errorOnExist: true, force: false }); return true; };
  if (exists(sourcePacks)) for (const entry of fs.readdirSync(sourcePacks, { withFileTypes: true })) { if (!entry.isFile() && !entry.isDirectory()) continue; if (copyPack(path.join(sourcePacks, entry.name), path.join(targetPacks, entry.name), entry.name)) resourcePacks.push(entry.name); }
  return { ok: true, source, target, mods, resourcePacks, unresolvedMods, skipped };
}
...
ipcMain.handle('transfer-version-content', async (_event, sourceVersion, targetVersion) => { try { const result = await transferVersionContent(sourceVersion, targetVersion); send('status', { type: 'success', message: `${result.mods.length} mod(s) and ${result.resourcePacks.length} resource pack(s) transferred from ${result.source} to ${result.target}.` }); return result; } catch (error) { return { ok: false, error: error.message }; } });
...
ipcMain.handle('search-free-music', async (_event, query, page = 1) => { try { return await searchFreeMusic(query, page); } catch (error) { return { ok: false, results: [], error: error.message }; } });
...
function modpackArchiveSafePath(relativePath) {
  ...
  if (!/^(mods|resourcepacks|config|shaderpacks|datapacks|options\.txt)(\/|$)/i.test(normalized)) return null;
  return normalized;
}
async function searchModpacks(query, gameVersion, page = 0) {
  const normalizedVersion = validModrinthVersion(gameVersion); const normalizedQuery = String(query || '').trim().slice(0, 80); const normalizedPage = Math.max(0, Math.min(99, Number(page) || 0));
  const facets = JSON.stringify([['project_type:modpack']]);
  const params = new URLSearchParams({ query: normalizedQuery, facets, limit: '18', offset: String(normalizedPage * 18), index: 'relevance' });
  const result = await modrinthJson(`${MODRINTH_API}/search?${params}`);
  return { results: (result.hits || []).map(hit => ({ projectId: hit.project_id, slug: hit.slug, title: hit.title, author: hit.author || '', description: hit.description || '', iconUrl: hit.icon_url || null, downloads: hit.downloads || 0, gameVersions: Array.isArray(hit.versions) ? hit.versions.slice(0, 8) : [], gameVersion: normalizedVersion || (Array.isArray(hit.versions) ? hit.versions[0] : '') || 'version detected on install' })).filter(item => item.projectId), page: normalizedPage, pageSize: 18, total: result.total_hits || 0, hasNext: (normalizedPage + 1) * 18 < (result.total_hits || 0) };
}
async function getModpackVersion(projectId, gameVersion) {
  const params = new URLSearchParams({ loaders: JSON.stringify(['fabric']), limit: '30' });
  if (validModrinthVersion(gameVersion)) params.set('game_versions', JSON.stringify([validModrinthVersion(gameVersion)]));
  const versions = await modrinthJson(`${MODRINTH_API}/project/${encodeURIComponent(projectId)}/version?${params}`);
  const selected = versions.find(version => version.version_type === 'release') || versions[0];
  if (!selected) throw new Error('No compatible Modrinth pack version found.');
  const file = selectPrimaryFile(selected.files, '.mrpack');
  if (!file) throw new Error('This Modrinth pack has no .mrpack file.');
  return { versionId: selected.id, versionNumber: selected.version_number, fileName: file.filename, downloadUrl: file.url, sha512: file.hashes?.sha512 || null };
}
async function fetchVerifiedBuffer(url, sha512 = null) {
  const response = await fetch(url, { headers: modrinthHeaders(), signal: AbortSignal.timeout(60000) }); if (!response.ok) throw new Error(`Download failed with ${response.status}.`);
  const buffer = Buffer.from(await response.arrayBuffer()); if (buffer.length > 250 * 1024 * 1024) throw new Error('The downloaded pack is too large.');
...
async function exportMrpackFile(version, metadata = {}) {
  ...
  const projectMap = installedProjectMap(normalized); const files = []; const missing = [];
  for (const folder of ['mods', 'resourcepacks']) { const dir = path.join(root, folder); if (!fs.existsSync(dir)) continue; for (const fileName of fs.readdirSync(dir)) { if (!/\.(jar|zip)$/i.test(fileName) || fileName.endsWith('.disabled')) continue; const record = Object.values(projectMap).find(item => projectRecordFileName(item) === fileName); if (!record?.downloadUrl) { missing.push(fileName); continue; } const local = fs.readFileSync(path.join(dir, fileName)); files.push({ path: `${folder}/${fileName}`, hashes: { sha512: crypto.createHash('sha512').update(local).digest('hex') }, env: { client: 'required', server: 'required' }, downloads: [record.downloadUrl], fileSize: local.length }); } }
  const packName = String(metadata.name || `Vortex ${normalized} Pack`).slice(0, 80); zip.file('modrinth.index.json', JSON.stringify({ game: 'minecraft', formatVersion: 1, versionId: String(metadata.version || '1.0.0'), name: packName, summary: String(metadata.summary || 'Created with Vortex Client Launcher').slice(0, 200), files, dependencies: { minecraft: normalized, fabric_loader: '' } }, null, 2));
  const result = await dialog.showSaveDialog(mainWindow, { title: 'Export Modrinth Modpack', defaultPath: `${packName.replace(/[^a-z0-9._-]+/gi, '-')}.mrpack`, filters: [{ name: 'Modrinth Modpack', extensions: ['mrpack'] }] }); if (result.canceled || !result.filePath) return { ok: false, canceled: true, missing }; await fs.promises.writeFile(result.filePath, await zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' })); return { ok: true, filePath: result.filePath, fileCount: files.length, missing };
}
ipcMain.handle('search-modpacks', async (_event, query, version, page) => { try { return { ok: true, ...(await searchModpacks(query, version, page)) }; } catch (error) { return { ok: false, error: error.message }; } });
ipcMain.handle('download-modpack', async (_event, version, pack) => { try { const compatible = await getModpackVersion(pack.projectId); const temp = path.join(app.getPath('temp'), `vortex-${Date.now()}.mrpack`); fs.writeFileSync(temp, await fetchVerifiedBuffer(compatible.downloadUrl, compatible.sha512)); const result = await installMrpackFile(version, temp); fs.rmSync(temp, { force: true }); const state = result.targetVersion ? saveState({ selectedVersion: result.targetVersion }) : loadState(); return { ...result, projectId: pack.projectId, versionNumber: compatible.versionNumber, selectedVersion: state.selectedVersion, state }; } catch (error) { return { ok: false, error: error.message }; } });
ipcMain.handle('import-mrpack', async (_event, version) => { try { const result = await dialog.showOpenDialog(mainWindow, { title: 'Import Modrinth Modpack', properties: ['openFile'], filters: [{ name: 'Modrinth Modpack', extensions: ['mrpack'] }] }); if (result.canceled || !result.filePaths[0]) return { ok: false, canceled: true }; const installed = await installMrpackFile(version, result.filePaths[0]); const state = installed.targetVersion ? saveState({ selectedVersion: installed.targetVersion }) : loadState(); return { ...installed, selectedVersion: state.selectedVersion, state }; } catch (error) { return { ok: false, error: error.message }; } });
